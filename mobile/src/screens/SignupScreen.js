...
export default function SignupScreen({ navigation }) {
  ...
  const submit = async () => {
    if (!email.trim() || !password || !confirm || !swKey.trim()) {
    ...
    setLoading(true);
    try {
      await signup(email.trim(), password, swKey.trim());
    } catch (e) {
  ...
  const field = (setter) => (v) => { setter(v); setError(''); };
  ...
  return (
    ...
    >
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputRow}>
          ...
          <TextInput
            style={styles.input}
            value={password}
            onChangeText={field(setPassword)}
        ...
        <Text style={[styles.label, { marginTop: 16 }]}>Splitwise API Key</Text>
        <Text style={styles.hint}>
          splitwise.com → Account → Your apps → Register your application
        </Text>
      ...
      <TouchableOpacity style={styles.switchRow} onPress={() => navigation.navigate('Login')}>
        ...
        <Text style={[styles.switchText, { color: INDIGO, fontWeight: '700' }]}>Sign In</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  hint:     { fontSize: 11, color: '#94a3b8', marginBottom: 0, lineHeight: 16 },
  inputRow: {
